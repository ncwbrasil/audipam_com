...
<div class='centro'>
    ...
    $sql = "SELECT * FROM cadastro_empresas 
			WHERE emp_cliente = :emp_cliente AND ".$nome_query."
    ...
    if($pagina == "cadastro_clientes")
    {
		...
		if ($rows > 0)
		{
			...
			<table align='center' width='100%' border='0' cellspacing='0' cellpadding='10' class='bordatabela'>
				<tr>
					<td class='titulo_first' align='center' width='1'>Cliente</td>
				...
				while($result = $stmt->fetch())
				{
					...
					<div id='user-options-$emp_id' class='toolbar-icons' style='display: none;'>
						...
						if($emp_cliente == 1)
						{
							...
							<a title='Tornar cliente -> empresa' onclick=\"
								abreMask(
									...
									'<input value=\' Sim \' type=\'button\' onclick=javascript:window.location.href=\'cadastro_clientes.php?pagina=cadastro_clientes&action=empresa&emp_id=$emp_id$autenticacao\';>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+
						...
						else
						{
							...
							<a title='Tornar empresa -> cliente' onclick=\"
								abreMask(
									...
									'<input value=\' Sim \' type=\'button\' onclick=javascript:window.location.href=\'cadastro_clientes.php?pagina=cadastro_clientes&action=cliente&emp_id=$emp_id$autenticacao\';>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+
									'<input value=\' Não \' type=\'button\' class=\'close_janela\'>');
								\">
								<img border='0' src='../imagens/icon-cliente.png'></i>
							</a>
						...
						if($emp_status == 1)
						{
							echo "<a title='Desativar' href='cadastro_clientes.php?pagina=cadastro_clientes&action=desativar&emp_id=$emp_id$autenticacao'><img border='0' src='../imagens/icon-ativa-desativa.png'></a>";
						}
						else
						{
							echo "<a title='Ativar' href='cadastro_clientes.php?pagina=cadastro_clientes&action=ativar&emp_id=$emp_id$autenticacao'><img border='0' src='../imagens/icon-ativa-desativa.png' ></a>";
						}
						echo "
						<a title='Editar' href='cadastro_clientes.php?pagina=cadastro_clientes_editar&emp_id=$emp_id$autenticacao'><img border='0' src='../imagens/icon-editar.png' ></a>
						<a title='Excluir' onclick=\"
							abreMask(
								'Deseja realmente excluir o cliente <b>$emp_nome_razao</b>?<br><br>'+
								'<input value=\' Sim \' type=\'button\' onclick=javascript:window.location.href=\'cadastro_clientes.php?pagina=cadastro_clientes&action=excluir&emp_id=$emp_id$autenticacao\';>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+
								'<input value=\' Não \' type=\'button\' class=\'close_janela\'>');
							\">
					...
					";
					echo "<tr class='$c1'>
							  <td><div style='background:url($emp_logo); height: 50px; width:50px; object-fit:cover; background-size:100%; border-radius:100px;'></div></td>
							  <td>";
								  if($emp_tipo == 'PJ')
								  {
				...
				echo "</table>";
				$variavel = "&pagina=cadastro_clientes&fil_nome=$fil_nome".$autenticacao."";
				$cnt = "SELECT COUNT(*) FROM cadastro_empresas WHERE emp_cliente = :emp_cliente AND ".$nome_query."";
    ...
    if($pagina == 'cadastro_clientes_editar')
    {
        $sql = "SELECT * FROM cadastro_empresas 
		...
		if($rows > 0)
        {
			...
			echo "
            <form name='form_cadastro_empresas' id='form_cadastro_empresas' enctype='multipart/form-data' method='post' action='cadastro_clientes.php?pagina=cadastro_clientes&action=editar&emp_id=$emp_id$autenticacao'>
								...
								<td align='left'>
									...
									else
									{
										...
										<div class='bloco_departamento'>
											...
											<p><label>CEP:</label> 				<input name='departamentos[1][dep_cep]' id='dep_cep' value='".$emp_cep."' class='dep_cep' placeholder='CEP' maxlength='9' onkeypress='mascaraCEP(this); return SomenteNumero(event);' />
											...
											<p><label>Bairro:</label> 			<input name='departamentos[1][dep_bairro]' id='dep_bairro' value='".$emp_bairro."' class='dep_bairro' placeholder='Bairro' />
											<p><label>Endereço:</label> 		<input name='departamentos[1][dep_endereco]' id='dep_endereco' value='".$emp_endereco."' class='dep_endereco' placeholder='Endereço' />
											<p><label>Número:</label> 			<input name='departamentos[1][dep_numero]' id='dep_numero' value='".$emp_numero."' class='dep_numero' placeholder='Número' />
											   <label>Complemento:</label> 		<input name='departamentos[1][dep_comp]' id='dep_comp' value='".$emp_comp."' placeholder='Complemento' />
