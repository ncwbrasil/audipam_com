<?php
session_start (); 
$pagina_link = 'relatorios_solicitacoes_status';
...
<div class='centro'>
    <div class='box'>
	<?php
    $page = "Relatórios &raquo; <a href='relatorios_solicitacoes_status.php?pagina=relatorios_solicitacoes_status".$autenticacao."'>Solicitações por Status</a> ";
	$agi_id = $_GET['agi_id'];
    ...
    $sql = "SELECT * FROM agenda_gerenciar_itens 
			...
			WHERE ".$empresa_query." AND ".$cliente_query." AND ".$contrato_query." AND ".$filtro_query." AND con_id IS NOT NULL
	...
	$stmt->execute();
    $rows = $stmt->rowCount();
    if($pagina == "relatorios_solicitacoes_status")
    {
        echo "
		...
		<div class='filtro'>
			...
			<form name='form_filtro' id='form_filtro' enctype='multipart/form-data' method='post' action='relatorios_solicitacoes_status.php?pagina=relatorios_solicitacoes_status".$autenticacao."&filtro=1'>
			...
			<input type='text' name='fil_data_inicio' id='fil_data_inicio' autocomplete='off' placeholder='Data Último Status Início' value='".implode('/',array_reverse(explode('-',$fil_data_inicio)))."' onkeypress='return mascaraData(this,event);'>
			<input type='text' name='fil_data_fim' id='fil_data_fim' autocomplete='off'  placeholder='Data Último Status Fim' value='".implode('/',array_reverse(explode('-',$fil_data_fim)))."' onkeypress='return mascaraData(this,event);'>
			<br>
			...
			if($filtro == 1)
			{
				echo "<img class='hand' title='Imprimir Relatório' style='float:right; margin:0 5px;' src='../imagens/icon-pdf.png' onclick=javascript:window.open('relatorio_solicitacoes_status_imprimir.php?fil_cliente=$fil_cliente&fil_contrato=$fil_contrato&fil_tecnico=".str_replace(" ","%20",$fil_tecnico)."&fil_contato=".str_replace(" ","%20",$fil_contato)."&fil_data_inicio=$fil_data_inicio&fil_data_fim=$fil_data_fim$autenticacao');>";
			}
			echo "
		</div>
		</div>
							  ...
							  <td>
								...
								$sql = "SELECT *,Year(sol_data_cadastro) as ano  FROM agenda_gerenciar_itens 
										...
										WHERE h1.sts_id = (SELECT MAX(h2.sts_id) FROM cliente_status_solicitacoes h2 where h2.sts_solicitacao = h1.sts_solicitacao) AND
											  ".$status_query." AND ".$empresa_query." AND ".$cliente_query." AND ".$contrato_query." AND ".$data_query." AND ".$filtro_query." AND con_id = :con_id
										GROUP BY sol_id  
										";
								...
								if($rows_sol > 0)
								{
									while($result_sol = $stmt_sol->fetch())
									{
										...
										echo "
										<div style='width:50%; float:left;'><b>Solicitação:</b> ".$result_sol['ano'].".".$result_sol['sol_id']." - <b>Data do Registro da Solicitação:</b> ".implode("/",array_reverse(explode("-",substr($result_sol['sol_data_cadastro'],0,10))))."</div><div style='width:50%; float:right; text-align:right;'><b>Última atualização:</b> ".implode("/",array_reverse(explode("-",substr($result_sol['sts_data'],0,10))))." às  ".implode("/",array_reverse(explode("-",substr($result_sol['sts_data'],11,5))))." - <u>".$sts_status."</u></div><br><b>Técnicos:</b> ".$usuarios."<br>
