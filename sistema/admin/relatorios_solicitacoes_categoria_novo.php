...
<div class='centro'>
    ...
    if($pagina == "relatorios_solicitacoes_categoria")
    {
		...
		<div class='filtro'>
			...
			<form name='form_filtro' id='form_filtro' target='_blank' enctype='multipart/form-data' method='post' action='relatorios_solicitacoes_categoria.php?pagina=relatorios_solicitacoes_categoria".$autenticacao."&filtro=1'>
			...
			if($filtro == 1)
			{
				echo "<img class='hand' title='Imprimir Relatório' style='float:right; margin:0 5px;' src='../imagens/icon-pdf.png' onclick=javascript:window.open('relatorio_solicitacoes_categoria_imprimir.php?fil_cliente=$fil_cliente&fil_contrato=$fil_contrato&fil_tecnico=".str_replace(" ","%20",$fil_tecnico)."&fil_contato=".str_replace(" ","%20",$fil_contato)."&fil_data_inicio=$fil_data_inicio&fil_data_fim=$fil_data_fim$autenticacao');>";
			}
			echo "
		</div>
		</div>
								...
								if($rows_cat > 0)
								{
									while($result_cat = $stmt_cat->fetch())
									{
										...
										$sql = "SELECT *,Year(sol_data_cadastro) as ano  FROM agenda_gerenciar_itens 
												...
												ON agenda_gerenciar_usuario.agu_agenda_item = agenda_gerenciar_itens.agi_id
												WHERE h1.sts_id = (SELECT MAX(h2.sts_id) FROM cliente_status_solicitacoes h2 where h2.sts_solicitacao = h1.sts_solicitacao) AND
													".$empresa_query." AND ".$cliente_query." AND ".$contrato_query." AND ".$data_query." AND ".$filtro_query." AND sol_categoria = :sol_categoria
												GROUP BY sol_id 
												";
										...
										if($rows_sol > 0)
										{
											while($result_sol = $stmt_sol->fetch())
											{
												...
												if($result_sol['sts_foto'] != "")
												{
													...
													<img src='".$result_sol['sts_foto']."' style='width:47%; float:left;'>
													";
												}if($result_sol['sts_foto2'] != "")
												{
													echo "
													<img src='".$result_sol['sts_foto2']."' style='width:47%; float:right;'>
													";
												}
												echo "
												</div>
