<div  class="modal fade in" id="exp_materiasVotacaoNominal<?php echo $id_exp_materias;?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" >
        ...
        <div class="modal-content">
            ...
            <div class="modal-body"  style="padding-left:0; padding-right:0; text-align:left; line-height:30px;">                
                <?php
                echo "	
                <form name='form_exp_materias'  enctype='multipart/form-data' method='post' action='cadastro_sessoes_plenarias_exp_materias/".$id."/view/registrar_votacao_nominal'>							
                    ...
                    while($result_int = $stmt_int->fetch())
                    {
                        ...
                        $sql = "SELECT *
                                FROM cadastro_sessoes_plenarias_exp_materias_votacao_nominal 
                                LEFT JOIN cadastro_sessoes_plenarias_exp_materias_votacao ON cadastro_sessoes_plenarias_exp_materias_votacao.id = cadastro_sessoes_plenarias_exp_materias_votacao_nominal.votacao
                                WHERE cadastro_sessoes_plenarias_exp_materias_votacao.materia_exp = :materia_exp AND 
                                      cadastro_sessoes_plenarias_exp_materias_votacao_nominal.parlamentar = :parlamentar
                                      AND cadastro_sessoes_plenarias_exp_materias_votacao_nominal.ativo = :ativo
                                ";
                        $stmt_par = $PDO_PROCLEGIS->prepare($sql);                           
                        $stmt_par->bindParam(':materia_exp', 	$id_exp_materias);                                
