    ...
    <div class="modal-dialog modal-lg" >
        ...
        <div class="modal-content">
            ...
            <div class="modal-body"  style="padding-left:0; padding-right:0; text-align:center;">                
                <?php
                echo "	
                <form name='form_tramitacao'  enctype='multipart/form-data' method='post' action='cadastro_materias/exib/".$id."/editar_tramitacao#tramitacao'>							
                    ...
                    <p><label>Status tramitação*:</label> <select name='status_tramitacao' id='status_tramitacao' class='obg' >
                        <option value='$status_tramitacao'>$nome_status</option>";
                            $sql = "SELECT *
                                    FROM aux_materias_status_tramitacao
                                    WHERE ativo = :ativo 
                                    ORDER BY nome ASC";
                            $stmt_int = $PDO_PROCLEGIS->prepare($sql);     
                            $stmt_int->bindValue(':ativo', 	1);                                
                    ...
                    <p><label>Turno:</label> <select name='turno' id='turno' >
                        <option value='$turno'>$turno</option>
                        <option value='Primeiro'>Primeiro</option>
                        <option value='Segundo'>Segundo</option>
                        <option value='Único'>Único</option>
                        <option value='Suplmentar'>Suplmentar</option>
                        <option value='Votação Única em Regime de Urgência'>Votação Única em Regime de Urgência</option>
                        <option value='Deliberação'>Deliberação</option>
                        <option value='1ª Votação'>1ª Votação</option>
                        <option value='2ª e 3ª Votações'>2ª e 3ª Votações</option>
                        <option value='1ª e 2ª Votações'>1ª e 2ª Votações</option>
                        <option value='1ª e 2ª Votações em Regime de Urgência'>1ª e 2ª Votações em Regime de Urgência</option>
                    </select>
                    ...
                    <p><label>Nome do documento:</label> <input name='nome_documento' value='$nome_documento' placeholder='Nome do documento' autocomplete='off' '>
                    <p><label>Página(s):</label> <input name='paginas' value='$paginas' placeholder='Exemplo: 5 - 9 ou 5, 6, 7, 8, 9' autocomplete='off' '>
                </form>
                ";
                ?>        
            </div>
