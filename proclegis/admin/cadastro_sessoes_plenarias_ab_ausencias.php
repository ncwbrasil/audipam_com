...
<body>
	<main class="cd-main-content">    
        ...
        $page = "Cadastro  &raquo; <a href='cadastro_sessoes_plenarias/view'>Sessões Plenárias</a> &raquo;  <a href='cadastro_sessoes_plenarias/exib/$id'>Exibir</a> ";
            ...
            if($action == "adicionar_ausencias")
            {            
                           
                $parlamentar   = $_POST['parlamentar'];
                $data  = reverteData($_POST['data']);
                $horario   = $_POST['horario'];
                $tipo_justificativa   = $_POST['tipo_justificativa'];
                ...
                $sql = "INSERT INTO cadastro_sessoes_plenarias_ab_ausencias SET ".bindFields($dados);
                $stmt = $PDO_PROCLEGIS->prepare($sql);	
            ...
            if($action == "editar_ausencias")
            {                       
                $id_ausencias   = $_POST['id_ausencias'];
                $parlamentar   = $_POST['parlamentar'];
                $data  = reverteData($_POST['data']);
                $horario   = $_POST['horario'];
                $tipo_justificativa   = $_POST['tipo_justificativa'];
                ...
                $sql = "UPDATE cadastro_sessoes_plenarias_ab_ausencias SET ".bindFields($dados)." WHERE id = :id ";
                $stmt = $PDO_PROCLEGIS->prepare($sql); 
                $dados['id'] =  $id_ausencias;
                if($stmt->execute($dados))
                {		
                    log_operacao($id, $PDO_PROCLEGIS); 
                    ?>
            ...
            if($action == 'excluir_ausencias')
            {
                $id_sub = $_GET['id_sub'];

                $sql = "UPDATE cadastro_sessoes_plenarias_ab_ausencias set ativo = :ativo WHERE id = :id";
                $stmt = $PDO_PROCLEGIS->prepare($sql);
            ...
            $sql = "SELECT *, cadastro_sessoes_plenarias_ab_ausencias.id as id
                    ...
                    WHERE cadastro_sessoes_plenarias_ab_ausencias.ativo = :ativo and ".$nome_query." AND cadastro_sessoes_plenarias_ab_ausencias.sessao_plenaria = :sessao_plenaria				
            ...
            if($pagina == "view")
            {
                include("../mod_includes/modal/ausenciasAdd.php");
                echo "
                <div class='titulo'> $page  &raquo; Justificativas de Ausência </div>
                <div id='botoes'>
                    <div class='g_adicionar' title='Adicionar' data-toggle='modal' data-target='#ausenciasAdd'><i class='fas fa-plus'></i></div>
                    <div class='filtrar'><span class='f'><i class='fas fa-filter'></i></span></div>
                    <div class='filtro'>
                        <form name='form_filtro' id='form_filtro' enctype='multipart/form-data' method='post' action='cadastro_sessoes_plenarias_ab_ausencias/$id/view'>
                                    ...
                                    <td align=center width='150'>
                                            <div class='g_excluir' title='Excluir' onclick=\"
                                                abreMask('<p class=\'titulo\'>Alerta</p><p><br><br>'+
                                                    ...
                                                    '<input value=\' Sim \' type=\'button\' class=\'close_janela\' onclick=verificaPermissao(".$permissoes["excluir"].",\'cadastro_sessoes_plenarias_ab_ausencias/$id/view/excluir_ausencias/$id_ausencias\');>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+
                                                    '<input value=\' Não \' type=\'button\' class=\'close_janela\'>');
                                                \">	<i class='far fa-trash-alt'></i>
                                            </div>
                                            <div class='g_editar' title='Editar' data-toggle='modal' data-target='#ausenciasEdit".$id_ausencias."'><i class='fas fa-pencil-alt'></i></div> 
                                            
                                    </td>
                                </tr>";
                                include("../mod_includes/modal/ausenciasEdit.php");
                        }
                        echo "</table>";
