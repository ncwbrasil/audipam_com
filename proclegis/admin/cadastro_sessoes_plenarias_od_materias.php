...
<body>
	<main class="cd-main-content">    
		...
		<div class="content-wrapper">
            ...
            if($action == "adicionar_od_materias")
            {            
                ...
                $sql = "INSERT INTO cadastro_sessoes_plenarias_od_materias SET ".bindFields($dados);
                $stmt = $PDO_PROCLEGIS->prepare($sql);	
            ...
            if($action == "editar_od_materias")
            {                       
                $id_od_materias   = $_POST['id_od_materias'];
                $tipo_materia   = $_POST['tipo_materia'];
                $materia   = $_POST['materia'];
                $ordem   = $_POST['ordem'];
                $tipo_votacao   = $_POST['tipo_votacao'];if($tipo_votacao == ""){$tipo_votacao = null;}
                ...
                $sql = "UPDATE cadastro_sessoes_plenarias_od_materias SET ".bindFields($dados)." WHERE id = :id ";
                $stmt = $PDO_PROCLEGIS->prepare($sql); 
                $dados['id'] =  $id_od_materias;
                if($stmt->execute($dados))
                {		
                    log_operacao($id, $PDO_PROCLEGIS); 	
                    ?>
            ...
            {                     
                ...
                $sql = "SELECT * FROM cadastro_sessoes_plenarias_od_materias WHERE sessao_plenaria = :sessao_plenaria AND status LIKE :status ";
                $sts = "%Aberta%";
                $stmt = $PDO_PROCLEGIS->prepare($sql); 
            ...
            {                     
                ...
                $sql = "SELECT * FROM cadastro_sessoes_plenarias_od_materias WHERE sessao_plenaria = :sessao_plenaria AND status LIKE :status AND ativo = :ativo";
                $sts = "%aberta%";
                $stmt = $PDO_PROCLEGIS->prepare($sql); 
                $stmt->bindParam(':sessao_plenaria', 	$id);    
                $stmt->bindParam(':status', 	$sts);     
            ...
            {                       
                ...
                {		      
                    ...
                    $sql = "SELECT * FROM cadastro_sessoes_plenarias_od_materias_leitura WHERE materia_od = :materia_od ";
                    $stmt = $PDO_PROCLEGIS->prepare($sql); 
                    $stmt->bindParam(':materia_od', 	$id_od_materias);       
                    $stmt->execute();
                    $rows = $stmt->rowCount();              
                    if($rows > 0)
                    {		
                        $result = $stmt->fetch();
                        // REGISTRA LEITURA
                        $sql = "UPDATE cadastro_sessoes_plenarias_od_materias_leitura SET 
                                       observacao = :observacao 
                                WHERE id = :id";
                        $stmt = $PDO_PROCLEGIS->prepare($sql); 
                    ...
                    {
                        // REGISTRA LEITURA
                        $sql = "INSERT INTO cadastro_sessoes_plenarias_od_materias_leitura SET 
                                materia_od = :materia_od,
                                observacao = :observacao ";
                        $stmt = $PDO_PROCLEGIS->prepare($sql); 
                        $stmt->bindParam(':materia_od', 	$id_od_materias);    
                        $stmt->bindParam(':observacao', 	$_POST['observacao']);    
            ...
            {                       
                ...
                {		      
                    ...
                    $sql = "SELECT * FROM cadastro_sessoes_plenarias_od_materias_votacao WHERE materia_od = :materia_od ";
                    $stmt = $PDO_PROCLEGIS->prepare($sql); 
                    $stmt->bindParam(':materia_od', 	$id_od_materias);       
                    $stmt->execute();
                    $rows = $stmt->rowCount();              
                    ...
                    {		
                        $result = $stmt->fetch();
                        // REGISTRA VOTACAO
                        $sql = "UPDATE cadastro_sessoes_plenarias_od_materias_votacao SET 
                                       sim = :sim,
                                       nao = :nao,
                                       abstencao = :abstencao,
                    ...
                    {
                        // REGISTRA VOTACAO
                        $sql = "INSERT INTO cadastro_sessoes_plenarias_od_materias_votacao SET 
                                materia_od = :materia_od,
                                sim = :sim,
                                nao = :nao,
                                abstencao = :abstencao,
                ...
                {		      
                    ...
                    else
                    {
                        ...
                        {		
                            ...
                            {
                                $sql = "INSERT INTO cadastro_sessoes_plenarias_od_materias_votacao_nominal SET 
                                                    votacao = :votacao,
            ...
            if($action == 'excluir_od_materias')
            {
                $id_sub = $_GET['id_sub'];

                $sql = "UPDATE cadastro_sessoes_plenarias_od_materias SET ativo = :ativo WHERE id = :id";
                $stmt = $PDO_PROCLEGIS->prepare($sql);
            ...
            if($action == 'excluir_leitura')
            {
                ...
                $sql = "UPDATE cadastro_sessoes_plenarias_od_materias_leitura set ativo = :ativo WHERE materia_od = :materia_od";
                $stmt = $PDO_PROCLEGIS->prepare($sql);
                $stmt->bindParam(':materia_od',$id_sub);
                $stmt->bindValue (':ativo',0);
                if($stmt->execute())
                {
                     // VOLTA STATUS LEITURA
                    $sql = "UPDATE cadastro_sessoes_plenarias_od_materias SET 
                                    status = :status,
                                    botao = :botao 
                            WHERE id = :id";
                    $stmt = $PDO_PROCLEGIS->prepare($sql); 
            ...
            if($action == 'excluir_votacao')
            {
                ...
                $sql = "UPDATE cadastro_sessoes_plenarias_od_materias_votacao set ativo = :ativo WHERE materia_od = :materia_od";
                $stmt = $PDO_PROCLEGIS->prepare($sql);
                $stmt->bindParam(':materia_od',$id_sub);
                $stmt->bindValue (':ativo',0); 
                if($stmt->execute())
            ...
            $sql = "SELECT *, cadastro_sessoes_plenarias_od_materias.id as id 
                            ...
                            , cadastro_sessoes_plenarias_od_materias.observacao as observacao
                            , cadastro_sessoes_plenarias_od_materias_leitura.observacao as observacao_leitura
                            , cadastro_sessoes_plenarias_od_materias_votacao.observacao as observacao_votacao
                    FROM cadastro_sessoes_plenarias_od_materias 
                    LEFT JOIN cadastro_sessoes_plenarias_od_materias_leitura ON cadastro_sessoes_plenarias_od_materias_leitura.materia_od = cadastro_sessoes_plenarias_od_materias.id                     
                    LEFT JOIN ( cadastro_sessoes_plenarias_od_materias_votacao 
                        LEFT JOIN aux_sessoes_plenarias_tipo_resultado ON aux_sessoes_plenarias_tipo_resultado.id = cadastro_sessoes_plenarias_od_materias_votacao.resultado)
                    ON cadastro_sessoes_plenarias_od_materias_votacao.materia_od = cadastro_sessoes_plenarias_od_materias.id
                    ...
                    WHERE cadastro_sessoes_plenarias_od_materias.ativo = :ativo and ".$nome_query." AND cadastro_sessoes_plenarias_od_materias.sessao_plenaria = :sessao_plenaria		
            ...
            if($pagina == "view")
            {
                include("../mod_includes/modal/od_materiasAdd.php");
                echo "
                <div class='titulo'> $page  &raquo; Matérias da Ordem do Dia </div>
                <div id='botoes'>
                    <div class='g_adicionar' title='Adicionar' data-toggle='modal' data-target='#od_materiasAdd'><i class='fas fa-plus'></i></div>
                    <div class='filtrar'><span class='f'><i class='fas fa-filter'></i></span></div>
                    <div class='filtro'>
                        <form name='form_filtro' id='form_filtro' enctype='multipart/form-data' method='post' action='cadastro_sessoes_plenarias_od_materias/$id/view'>
                        ...
                        while($result = $stmt->fetch())
                        {
                            ...
                            echo "<tr class='$c1'>
                                   ...
                                   <td align='center'><span class='bold'>".$status."</span><p>
                                        ...
                                        {
                                            $function = "onclick=verificaPermissao(".$permissoes["edit"].",'cadastro_sessoes_plenarias_od_materias/$id/view/abrir_leitura/$id_od_materias');";                                            
                                        ...
                                        if($tipo_votacao != "Nominal")
                                        {
                                            ...
                                            {
                                                $function = "onclick=verificaPermissao(".$permissoes["edit"].",'cadastro_sessoes_plenarias_od_materias/$id/view/abrir_votacao/$id_od_materias');";                                            
                                                $cor = "style='background:#F23B3B;'";
                                            ...
                                            {
                                                $function = "data-toggle='modal' data-target='#od_materiasVotacao".$id_od_materias."'";  
                                        ...
                                        else if($tipo_votacao == "Nominal")
                                        {
                                            ...
                                            elseif($botao == "Registrar votação" || $botao == "Ver resultado votação" )
                                            {
                                                $function = "data-toggle='modal' data-target='#od_materiasVotacaoNominal".$id_od_materias."'";  
                                                if($botao == "Ver resultado votação")
                                                {
                                                    $cor = "style='background:#1DBA9B;'";
                                                }                                                                  
                                    ...
                                    <td align=center width='150'>
                                            <div class='g_excluir' title='Excluir' onclick=\"
                                                abreMask('<p class=\'titulo\'>Alerta</p><p><br><br>'+
                                                    ...
                                                    '<input value=\' Sim \' type=\'button\' class=\'close_janela\' onclick=verificaPermissao(".$permissoes["excluir"].",\'cadastro_sessoes_plenarias_od_materias/$id/view/excluir_od_materias/$id_od_materias\');>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+
                                                    '<input value=\' Não \' type=\'button\' class=\'close_janela\'>');
                                                \">	<i class='far fa-trash-alt'></i>
                                            </div>
                                            <div class='g_editar' title='Editar' data-toggle='modal' data-target='#od_materiasEdit".$id_od_materias."'><i class='fas fa-pencil-alt'></i></div> 
                                            
                                    </td>
                                </tr>";
                                include("../mod_includes/modal/od_materiasEdit.php");
                                include("../mod_includes/modal/od_materiasLeitura.php");
                                include("../mod_includes/modal/od_materiasVotacao.php");
                                include("../mod_includes/modal/od_materiasVotacaoNominal.php");
                        }
                        echo "</table>";
