...
<body>
	<main class="cd-main-content">    
		...
		<div class="content-wrapper">
            ...
            if($action == "adicionar_retirada_pauta")
            {            
                           
                $parlamentar   = $_POST['parlamentar'];
                $data  = reverteData($_POST['data']);
                $materia_ordem_dia   = $_POST['materia_ordem_dia'];if($materia_ordem_dia == ""){$materia_ordem_dia = null;}
                $materia_expediente   = $_POST['materia_expediente'];if($materia_expediente == ""){$materia_expediente = null;}
                ...
                $dados = array(
                    ...
                    'materia_ordem_dia' 		    => $materia_ordem_dia,
                    'materia_expediente' 		    => $materia_expediente,
                    'observacao' 		        => $observacao
                    );
                $sql = "INSERT INTO cadastro_sessoes_plenarias_ab_retirada_pauta SET ".bindFields($dados);
                $stmt = $PDO_PROCLEGIS->prepare($sql);	
            ...
            if($action == "editar_retirada_pauta")
            {                       
                $id_retirada_pauta   = $_POST['id_retirada_pauta'];
                $parlamentar   = $_POST['parlamentar'];
                $data  = reverteData($_POST['data']);
                $materia_ordem_dia   = $_POST['materia_ordem_dia'];if($materia_ordem_dia == ""){$materia_ordem_dia = null;}
                $materia_expediente   = $_POST['materia_expediente'];if($materia_expediente == ""){$materia_expediente = null;}
                ...
                $sql = "UPDATE cadastro_sessoes_plenarias_ab_retirada_pauta SET ".bindFields($dados)." WHERE id = :id ";
                $stmt = $PDO_PROCLEGIS->prepare($sql); 
                $dados['id'] =  $id_retirada_pauta;
                if($stmt->execute($dados))
                {	
                    log_operacao($id, $PDO_PROCLEGIS); 	
            ...
            if($action == 'excluir_retirada_pauta')
            {
                $id_sub = $_GET['id_sub'];

                $sql = "UPDATE cadastro_sessoes_plenarias_ab_retirada_pauta set ativo = :ativo WHERE id = :id";
                $stmt = $PDO_PROCLEGIS->prepare($sql);
            ...
            $sql = "SELECT *, cadastro_sessoes_plenarias_ab_retirada_pauta.id as id
                    ...
                    WHERE cadastro_sessoes_plenarias_ab_retirada_pauta.ativo = :ativo and ".$nome_query." AND cadastro_sessoes_plenarias_ab_retirada_pauta.sessao_plenaria = :sessao_plenaria	
            ...
            if($pagina == "view")
            {
                include("../mod_includes/modal/retirada_pautaAdd.php");
                echo "
                <div class='titulo'> $page  &raquo; Retirada de Pauta </div>
                <div id='botoes'>
                    <div class='g_adicionar' title='Adicionar' data-toggle='modal' data-target='#retirada_pautaAdd'><i class='fas fa-plus'></i></div>
                    <div class='filtrar'><span class='f'><i class='fas fa-filter'></i></span></div>
                    <div class='filtro'>
                        <form name='form_filtro' id='form_filtro' enctype='multipart/form-data' method='post' action='cadastro_sessoes_plenarias_ab_retirada_pauta/$id/view'>
                                    ...
                                    <td align=center width='150'>
                                            <div class='g_excluir' title='Excluir' onclick=\"
                                                abreMask('<p class=\'titulo\'>Alerta</p><p><br><br>'+
                                                    ...
                                                    '<input value=\' Sim \' type=\'button\' class=\'close_janela\' onclick=verificaPermissao(".$permissoes["excluir"].",\'cadastro_sessoes_plenarias_ab_retirada_pauta/$id/view/excluir_retirada_pauta/$id_retirada_pauta\');>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+
                                                    '<input value=\' Não \' type=\'button\' class=\'close_janela\'>');
                                                \">	<i class='far fa-trash-alt'></i>
                                            </div>
                                            <div class='g_editar' title='Editar' data-toggle='modal' data-target='#retirada_pautaEdit".$id_retirada_pauta."'><i class='fas fa-pencil-alt'></i></div> 
                                            
                                    </td>
                                </tr>";
                                include("../mod_includes/modal/retirada_pautaEdit.php");
                        }
                        echo "</table>";
