...
<body>
	<main class="cd-main-content">    
		...
		<div class="content-wrapper">
            ...
            if($action == "adicionar_od_oradores")
            {            
                ...
                $sql = "INSERT INTO cadastro_sessoes_plenarias_od_oradores SET ".bindFields($dados);
                $stmt = $PDO_PROCLEGIS->prepare($sql);	
            ...
            if($action == "editar_od_oradores")
            {                       
                $id_od_oradores   = $_POST['id_od_oradores'];
                $parlamentar   = $_POST['parlamentar'];
                $ordem   = $_POST['ordem'];
                $url_video   = $_POST['url_video'];if($url_video == ""){$url_video = null;}
                $observacao   = $_POST['observacao'];if($observacao == ""){$observacao = null;}
                ...
                $sql = "UPDATE cadastro_sessoes_plenarias_od_oradores SET ".bindFields($dados)." WHERE id = :id ";
                $stmt = $PDO_PROCLEGIS->prepare($sql); 
                $dados['id'] =  $id_od_oradores;
                if($stmt->execute($dados))
                {		
                    log_operacao($id, $PDO_PROCLEGIS); 	
                    ?>
            ...
            if($action == 'excluir_od_oradores')
            {
                $id_sub = $_GET['id_sub'];

                $sql = "UPDATE cadastro_sessoes_plenarias_od_oradores set ativo = :ativo WHERE id = :id";
                $stmt = $PDO_PROCLEGIS->prepare($sql);
            ...
            $sql = "SELECT *, cadastro_sessoes_plenarias_od_oradores.id as id
                    ...
                    WHERE cadastro_sessoes_plenarias_od_oradores.ativo = :ativo and ".$nome_query." AND cadastro_sessoes_plenarias_od_oradores.sessao_plenaria = :sessao_plenaria	
            ...
            if($pagina == "view")
            {
                include("../mod_includes/modal/od_oradoresAdd.php");
                echo "
                <div class='titulo'> $page  &raquo; Oradores da Ordem do Dia </div>
                <div id='botoes'>
                    <div class='g_adicionar' title='Adicionar' data-toggle='modal' data-target='#od_oradoresAdd'><i class='fas fa-plus'></i></div>
                    <div class='filtrar'><span class='f'><i class='fas fa-filter'></i></span></div>
                    <div class='filtro'>
                        <form name='form_filtro' id='form_filtro' enctype='multipart/form-data' method='post' action='cadastro_sessoes_plenarias_od_oradores/$id/view'>
                                    ...
                                    <td align=center width='150'>
                                            <div class='g_excluir' title='Excluir' onclick=\"
                                                abreMask('<p class=\'titulo\'>Alerta</p><p><br><br>'+
                                                    ...
                                                    '<input value=\' Sim \' type=\'button\' class=\'close_janela\' onclick=verificaPermissao(".$permissoes["excluir"].",\'cadastro_sessoes_plenarias_od_oradores/$id/view/excluir_od_oradores/$id_od_oradores\');>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+
                                                    '<input value=\' Não \' type=\'button\' class=\'close_janela\'>');
                                                \">	<i class='far fa-trash-alt'></i>
                                            </div>
                                            <div class='g_editar' title='Editar' data-toggle='modal' data-target='#od_oradoresEdit".$id_od_oradores."'><i class='fas fa-pencil-alt'></i></div> 
                                            
                                    </td>
                                </tr>";
                                include("../mod_includes/modal/od_oradoresEdit.php");
                        }
                        echo "</table>";
