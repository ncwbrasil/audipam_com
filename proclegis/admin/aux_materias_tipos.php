...
<body>
	<main class="cd-main-content">    
		...
		<div class="content-wrapper">
            ...
            <?php                     
            $page = "Auxiliares &raquo; <a href='aux_materias/view'>Matérias Legislativas</a> &raquo; <a href='aux_materias_tipos/view'>Tipos de Matéria Legislativa</a>";
            ...
            {                       
                ...
                $sql = "INSERT INTO aux_materias_tipos SET ".bindFields($dados);
                    ...
                    foreach($_FILES as $key => $files)
                    {
                        ...
                        if(!empty($files_test))
                        {
                            ...
                            if(!empty($files["name"]["logo"]))
                            {
                                ...
                                $sql = "UPDATE aux_materias_tipos SET 
                                        logo 	 = :logo
                                        WHERE id = :id ";
                                $stmt = $PDO_PROCLEGIS->prepare($sql);
            ...
            if($action == 'editar')
            {
                $sql = "UPDATE aux_materias_tipos SET ".bindFields($dados)." WHERE id = :id ";
                $stmt = $PDO_PROCLEGIS->prepare($sql); 
                $dados['id'] =  $id;
                if($stmt->execute($dados))
                {
            ...
            if($action == 'excluir')
            {
                ...
                $sql = "UPDATE aux_materias_tipos set ativo = :ativo WHERE id = :id";
                $stmt = $PDO_PROCLEGIS->prepare($sql);
                $stmt->bindParam(':id',$id);
            ...
            if($action == 'ativar')
            {
                $sql = "UPDATE aux_materias_tipos SET status = :status WHERE id = :id ";
                $stmt = $PDO_PROCLEGIS->prepare($sql);
            ...
            $sql = "SELECT * FROM aux_materias_tipos 
                    WHERE aux_materias_tipos.ativo = :ativo and ".$sigla_query." 			
                    ORDER BY id DESC
            ...
            if($pagina == "view")
            {
                ...
                <div id='botoes'>
                    <div class='g_adicionar' title='Adicionar' onclick='verificaPermissao(".$permissoes["add"].",\"aux_materias_tipos/add\");'><i class='fas fa-plus'></i></div>
                    <div class='filtrar'><span class='f'><i class='fas fa-filter'></i></span></div>
                    <div class='filtro'>
                        <form name='form_filtro' id='form_filtro' enctype='multipart/form-data' method='post' action='aux_materias_tipos/view'>
                                    ...
                                    <td align=center>
                                            <div class='g_excluir' title='Excluir' onclick=\"
                                                abreMask('<p class=\'titulo\'>Alerta</p><p><br><br>'+
                                                    ...
                                                    '<input value=\' Sim \' type=\'button\' class=\'close_janela\' onclick=verificaPermissao(".$permissoes["excluir"].",\'aux_materias_tipos/view/excluir/$id?pag=$pag\');>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+
                                                    '<input value=\' Não \' type=\'button\' class=\'close_janela\'>');
                                                \">	<i class='far fa-trash-alt'></i>
                                            </div>
                                            <div class='g_editar' title='Editar' onclick='verificaPermissao(".$permissoes["edit"].",\"aux_materias_tipos/edit/$id?pag=$pag\");'><i class='fas fa-pencil-alt'></i></div>                                            
                                    </td>
                                </tr>";
                        }
                        echo "</table>";
                        $cnt = "SELECT COUNT(*) FROM aux_materias_tipos  WHERE ".$sigla_query."";
            ...
            {
                echo "	
                <form name='form' id='form' enctype='multipart/form-data' method='post' action='aux_materias_tipos/view/adicionar'>
                    ...
                    <ul class='nav nav-tabs'>
                        ...
                        <input type='button' id='botao_cancelar' onclick=javascript:window.location.href='aux_materias_tipos/view'; value='Cancelar'/></center>
                        </center>
                    </div>
                </form>
                ";
            ...
            {            		
                ...
                if($rows > 0)
                {
                    ...
                    echo "
                    <form name='form' id='form' enctype='multipart/form-data' method='post' action='aux_materias_tipos/view/editar/$id'>
