...
<body>
	<main class="cd-main-content">    
		...
		<div class="content-wrapper">
            ...
            <?php                     
            $page = "Cadastro &raquo; <a href='cadastro_parlamentares/view'>Parlamentares</a>";
            if(isset($_GET['id'])){$id = $_GET['id'];}
            $nome   = $_POST['nome'];
            $apelido   = $_POST['apelido'];
            $nivel_instrucao  = $_POST['nivel_instrucao'];if($nivel_instrucao == ""){$nivel_instrucao = null;}
            $sexo  = $_POST['sexo'];
            $data_nasc  = reverteData($_POST['data_nasc']);if($data_nasc == ""){$data_nasc = null;}
            $cpf    = $_POST['cpf'];
            $rg    = $_POST['rg'];
            $titulo_eleitor    = $_POST['titulo_eleitor'];
            $situacao_militar    = $_POST['situacao_militar'];if($situacao_militar == ""){$situacao_militar = null;}
            ...
            $cep = $_POST['cep'];
            $uf = $_POST['uf'];if($uf == ""){$uf = null;}
            $municipio = $_POST['municipio'];if($municipio == ""){$municipio = null;}
            $bairro = $_POST['bairro'];
            ...
            if($action == "adicionar_mandatos")
            {                       
                $legislatura   = $_POST['legislatura'];
                $coligacao   = $_POST['coligacao'];if($coligacao == ""){ $coligacao = null;}
                $votos  = $_POST['votos'];if($votos == ""){ $votos = null;}
                $data_inicio_mandato  = reverteData($_POST['data_inicio_mandato']);
                $data_fim_mandato  = reverteData($_POST['data_fim_mandato']);
                $tipo_afastamento    = $_POST['tipo_afastamento'];if($tipo_afastamento == ""){ $tipo_afastamento = null;}
                ...
                $sql = "INSERT INTO cadastro_parlamentares_mandatos SET ".bindFields($dados);
                $stmt = $PDO_PROCLEGIS->prepare($sql);	
                if($stmt->execute($dados))            
                {		
                    $id = $PDO_PROCLEGIS->lastInsertId(); 
            ...
            if($action == "editar_mandatos")
            {                       
                $id_mandato   = $_POST['id_mandato'];
                $legislatura   = $_POST['legislatura'];
                $coligacao   = $_POST['coligacao'];if($coligacao == ""){$coligacao = null;} 
                $votos  = $_POST['votos'];if($votos == ""){$votos = null;} 
                $data_inicio_mandato  = reverteData($_POST['data_inicio_mandato']);if($data_inicio_mandato == ""){$data_inicio_mandato = null;}   
                $data_fim_mandato  = reverteData($_POST['data_fim_mandato']);if($data_fim_mandato == ""){$data_fim_mandato = null;}   
                $tipo_afastamento    = $_POST['tipo_afastamento'];if($tipo_afastamento == ""){$tipo_afastamento = null;}   
                $observacao    = $_POST['observacao'];
                ...
                $sql = "UPDATE cadastro_parlamentares_mandatos SET ".bindFields($dados)." WHERE id = :id ";
                $stmt = $PDO_PROCLEGIS->prepare($sql); 
                $dados['id'] =  $id_mandato;
                if($stmt->execute($dados))
                {		
                    log_operacao($id_mandato, $PDO_PROCLEGIS);  
            ...
            if($action == 'excluir_mandato')
            {
                $id_sub = $_GET['id_sub'];
                $sql = "UPDATE cadastro_parlamentares_mandatos ativo = :ativo WHERE id = :id";
                $stmt = $PDO_PROCLEGIS->prepare($sql);
            ...
            if($action == "adicionar_filiacoes")
            {                       
                $partido   = $_POST['partido'];
                $data_filiacao  = reverteData($_POST['data_filiacao']);if($data_filiacao == ""){$data_filiacao = null;}   
                $data_desfiliacao  = reverteData($_POST['data_desfiliacao']);if($data_desfiliacao == ""){$data_desfiliacao = null;}  
                ...
                $sql = "INSERT INTO cadastro_parlamentares_filiacoes SET ".bindFields($dados);
                $stmt = $PDO_PROCLEGIS->prepare($sql);	
                if($stmt->execute($dados))
                {		
                    $id_filiacoes = $PDO_PROCLEGIS->lastInsertId(); 
                    log_operacao($id_filiacoes, $PDO_PROCLEGIS); 
            ...
            if($action == "editar_filiacoes")
            {                       
                $id_filiacoes   = $_POST['id_filiacoes'];
                $partido   = $_POST['partido'];
                $data_filiacao  = reverteData($_POST['data_filiacao']);if($data_filiacao == ""){$data_filiacao = null;}   
                $data_desfiliacao  = reverteData($_POST['data_desfiliacao']);if($data_desfiliacao == ""){$data_desfiliacao = null;}   
                ...
                $sql = "UPDATE cadastro_parlamentares_filiacoes SET ".bindFields($dados)." WHERE id = :id ";
                $stmt = $PDO_PROCLEGIS->prepare($sql); 
                $dados['id'] =  $id_filiacoes;
                if($stmt->execute($dados))
                {		
                    log_operacao($id_filiacoes, $PDO_PROCLEGIS);  
            ...
            if($action == 'excluir_filiacoes')
            {                
                $id_sub = $_GET['id_sub'];
                
                $sql = "UPDATE cadastro_parlamentares_filiacoes SET ativo = :ativo WHERE id = :id";
                $stmt = $PDO_PROCLEGIS->prepare($sql);
                $stmt->bindParam(':id',$id_sub);
                $stmt->bindValue(':ativo', 0); 
                if($stmt->execute())
            ...
            if($action == "adicionar_dependentes")
            {                       
                ...
                $sql = "INSERT INTO cadastro_parlamentares_dependentes SET ".bindFields($dados);
                $stmt = $PDO_PROCLEGIS->prepare($sql);	
                if($stmt->execute($dados))
                {	
                    $id = $PDO_PROCLEGIS->lastInsertId(); 
            ...
            if($action == "editar_dependentes")
            {                       
                $id_dependentes   = $_POST['id_dependentes'];
                $tipo_dependente   = $_POST['tipo_dependente'];
                $nome   = $_POST['nome'];
                $sexo   = $_POST['sexo'];
                $data_nasc  = reverteData($_POST['data_nasc']);if($data_nasc == ""){$data_nasc = null;}            
                ...
                $sql = "UPDATE cadastro_parlamentares_dependentes SET ".bindFields($dados)." WHERE id = :id ";
                $stmt = $PDO_PROCLEGIS->prepare($sql); 
                $dados['id'] =  $id_dependentes;
                if($stmt->execute($dados))
                {		
                    log_operacao($id_dependentes, $PDO_PROCLEGIS);  
            ...
            if($action == 'excluir_dependentes')
            {                
                $id_sub = $_GET['id_sub'];
                
                $sql = "UPDATE cadastro_parlamentares_dependentes ativo = :ativo WHERE id = :id";
                $stmt = $PDO_PROCLEGIS->prepare($sql);
                $stmt->bindParam(':id',$id_sub);
                $stmt->bindValue(':ativo', 0);
                if($stmt->execute())
                    ...
                    <form name='form' id='form' enctype='multipart/form-data' method='post' action='cadastro_parlamentares/view/editar/$id'>
                        ...
                        <div class='tab-content'>
                            <div id='dados_gerais' class='tab-pane fade in active'>
                                ...
                                <p><label>Email:</label> <input name='email' id='email' value='$email' placeholder='Email'>
            ...
            if($pagina == 'exib')
            {            		
                include("../mod_includes/modal/mandatosAdd.php");
                include("../mod_includes/modal/filiacoesAdd.php");
                include("../mod_includes/modal/dependentesAdd.php");
                ...
                if($rows > 0)
                {
                    ...
                    echo "
                        ...
                        <ul class='nav nav-tabs'>
                            ...
                            <li><a data-toggle='tab' href='#dependentes' id='dependentes-tab'>Dependentes</a></li>                 
                        </ul>
                        <div class='tab-content'>
                            <div id='dados_gerais' class='tab-pane fade in active'>
                            ...
                            <div id='mandatos' class='tab-pane fade in'>
                                ";
                                $sql = "SELECT *, YEAR(data_inicio) as data_inicio, YEAR(data_fim),
                                ...
                                echo "
                                <div id='botoes'>
                                    <div class='g_adicionar' title='Adicionar' data-toggle='modal' data-target='#mandatosAdd'><i class='fas fa-plus'></i></div>
                                </div>";
                                if ($rows > 0)
                                {
                                    echo "
                                            ...
                                            echo "<tr class='$c1'>
                                                    ...
                                                    <td align=center>
                                                            <div class='g_excluir' title='Excluir' onclick=\"
                                                                abreMask('<p class=\'titulo\'>Alerta</p><p><br><br>'+
                                                                    'Essa operação não poderá ser desfeita. Deseja realmente excluir este item? <br><br>'+
                                                                    '<input value=\' Sim \' type=\'button\' class=\'close_janela\' onclick=verificaPermissao(".$permissoes["excluir"].",\'".$pagina_link."/exib/$id/excluir_mandato/$id_mandato#mandatos\');>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+
                                                                    '<input value=\' Não \' type=\'button\' class=\'close_janela\'>');
                                                                \">	<i class='far fa-trash-alt'></i>
                                                            </div>
                                                            <div class='g_editar' title='Editar' data-toggle='modal' data-target='#mandatosEdit".$id_mandato."'><i class='fas fa-pencil-alt'></i></div>                                                                                                                        
                                                    </td>
                                                </tr>";
                                                include("../mod_includes/modal/mandatosEdit.php");
                                        }
                            ...
                            <div id='filiacoes' class='tab-pane fade in'>
                                ";
                                $sql = "SELECT *,                                         
                                ...
                                echo "
                                <div id='botoes'>
                                    <div class='g_adicionar' title='Adicionar' data-toggle='modal' data-target='#filiacoesAdd'><i class='fas fa-plus'></i></div>
                                </div>";
                                if ($rows > 0)
                                {
                                    echo "
                                            ...
                                            echo "<tr class='$c1'>
                                                    ...
                                                    <td align=center>
                                                            <div class='g_excluir' title='Excluir' onclick=\"
                                                                abreMask('<p class=\'titulo\'>Alerta</p><p><br><br>'+
                                                                    'Essa operação não poderá ser desfeita. Deseja realmente excluir este item? <br><br>'+
                                                                    '<input value=\' Sim \' type=\'button\' class=\'close_janela\' onclick=verificaPermissao(".$permissoes["excluir"].",\'".$pagina_link."/exib/$id/excluir_filiacoes/$id_filiacoes#filiacoes\');>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+
                                                                    '<input value=\' Não \' type=\'button\' class=\'close_janela\'>');
                                                                \">	<i class='far fa-trash-alt'></i>
                                                            </div>
                                                            <div class='g_editar' title='Editar' data-toggle='modal' data-target='#filiacoesEdit".$id_filiacoes."'><i class='fas fa-pencil-alt'></i></div>                                                                                                                        
                                                    </td>
                                                </tr>";
                                                include("../mod_includes/modal/filiacoesEdit.php");
                                        }
                            ...
                            <div id='dependentes' class='tab-pane fade in'>
                                ";
                                $sql = "SELECT *,                                         
                                        cadastro_parlamentares_dependentes.id as id_dependentes
                                        FROM cadastro_parlamentares_dependentes 
                                        LEFT JOIN aux_parlamentares_tipo_dependentes ON aux_parlamentares_tipo_dependentes.id = cadastro_parlamentares_dependentes.tipo_dependente
                                        WHERE parlamentar = :parlamentar
                                        ORDER BY cadastro_parlamentares_dependentes.id DESC
                                       ";
                                $stmt = $PDO_PROCLEGIS->prepare($sql);    
                                $stmt->bindParam(':fil_nome1', 	$fil_nome1);
                                $stmt->bindParam(':parlamentar', 	$id);                                    
                            ...
                            <br>               				
                            <center>                                                        
                                <input type='button' id='botao_cancelar' onclick=javascript:window.location.href='cadastro_parlamentares/view'; value='Voltar'/></center>
                            </center>
                        </div>
                    ";
                }
