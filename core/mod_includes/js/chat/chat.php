...
function chatHeartbeat() {
	...
	$sql = "select * from social_mensagens 
			LEFT JOIN cadastro_usuarios ON cadastro_usuarios.usu_id = social_mensagens.msg_remetente
			where (msg_destinatario = '".mysql_real_escape_string($_SESSION['user_id'])."' AND msg_lida = 0) order by msg_id ASC";
...
EOD;
	...
	$sql = "update social_mensagens set msg_lida = 1 where msg_destinatario = '".mysql_real_escape_string($_SESSION['user_id'])."' and msg_lida = 0";
	$query = mysql_query($sql);
...
function sendChat() {
	$msg_remetente = $_SESSION['user_id'];
